...
void save_embedding(ui* embedding, size_t max_depth, size_t embedding_cnt) {
    auto mode = embedding_cnt > 1 ? std::ios_base::app | std::ios_base::out : std::ios_base::out;
    auto path = std::filesystem::current_path().append(".tmp").append("embs.txt");
    std::ofstream embedding_file(path, mode);
    // std::cout << "emb #" << embedding_cnt << " ";
    for (size_t i = 0; i < max_depth; ++i) {
        embedding_file << embedding[i] << " ";
    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == max_depth - 1) {
                ...
                save_embedding(embedding, max_depth, embedding_cnt);
        ...
        for (ui i = 0; i < idx_count[u]; ++i) {
            ...
            if (!visited_vertices[v]) {
                ...
                if (depth != max_depth - 1) {
                    ...
                    embedding_count += 1;
                    save_embedding(embedding, max_depth, embedding_count);
                    child_fs.set();
                }
                visited_vertices[v] = false;
                reverse_embedding.erase(v);
